...
- [X] T032 Create Dockerfile at repository root — `FROM python:3.12-slim`; `COPY pyproject.toml uv.lock ./` and `COPY openkb/ ./openkb/`; `RUN uv pip install --no-cache-dir ".[api]"`; `EXPOSE 8000`; `CMD ["openkb", "serve", "--host", "0.0.0.0", "--port", "8000"]`; add `.dockerignore` excluding `.git`, `__pycache__`, `*.pyc`, `tests/`, `specs/`, `.env`
