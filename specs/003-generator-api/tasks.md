...
- [x] T001 Create `generator_api/` package: `__init__.py` with `__version__ = "0.1.0"` and docstring; no other content yet
...
- [x] T002 [P] Create `generator_api/config.py`: `pydantic-settings` `Settings` class with all env vars from `specs/003-generator-api/contracts/env-config.md` (`DATABASE_URL`, `AZURE_STORAGE_CONNECTION_STRING`, `AZURE_KB_CONTAINER`, `LLM_API_KEY`, `SIDECAR_STARTUP_TIMEOUT`, `GENERATOR_REQUEST_TIMEOUT`, `SCRATCH_DIR_ROOT`, `GENERATOR_HOST`, `GENERATOR_PORT`); `lru_cache`-wrapped `get_settings()`

- [x] T003 [P] Create `generator_api/models.py`: Pydantic v2 `QueryRequest` (`question: str` non-empty after strip max 8000 chars, `save: bool = False`); `QueryResponse` (`answer: str`, `citations: list[Any]`, `tokens_used: int`); `HealthResponse` (`status`, `postgres`, `azurite`, optional `detail`)

- [x] T004 [P] Create `generator_api/db.py`: async SQLAlchemy engine created from `settings.database_url`; `get_db()` async generator yielding an `AsyncSession`; `check_postgres()` coroutine that runs `SELECT 1` and returns `"ok"` or `"error: {msg}"`
