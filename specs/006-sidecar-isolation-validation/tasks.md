...
- [ ] T001 Create `tests/isolation/` directory structure: `__init__.py` at `tests/isolation/`, `tests/isolation/helpers/__init__.py`, and `tests/isolation/fixtures/kb_a/`, `tests/isolation/fixtures/kb_b/` directories (empty `__init__.py` files where needed)

- [ ] T002 Add `isolation-tests` optional dependency group to `pyproject.toml`: `pytest==9.0.3`, `pytest-asyncio==1.3.0`, `asyncpg==0.29.0`, `httpx==0.28.1`, `azure-storage-blob==12.24.0`, `psutil==6.0.0` (use versions already pinned in project where available); run `uv sync --extra isolation-tests` to lock
...
- [ ] T003 Create KB fixture source documents:
...
- [ ] T004 [P] Create `tests/isolation/helpers/process_helpers.py`: `wait_for_http(url, timeout)` polls GET until 200 or raises `TimeoutError`; `assert_port_bound(port)` uses `psutil.net_connections()` to assert a LISTEN socket exists on given port; `assert_proc_dead(pid)` asserts `psutil.pid_exists(pid)` is False; `list_scratch_dirs(scratch_root)` returns all subdirectories of `scratch_root`; `get_listening_ports()` returns set of all ports with LISTEN state

- [ ] T005 [P] Create `tests/isolation/helpers/blob_helpers.py`: `seed_wiki_blobs(connection_string, kb_id, kb_slug)` async function uploads the pre-compiled wiki blobs for a KB fixture to Azurite per `contracts/kb-fixture-schema.md` (summary.md + concept page); `list_kb_blobs(connection_string, kb_id)` returns list of blob paths under `kb-{kb_id}/`; `delete_kb_blobs(connection_string, kb_id)` removes all blobs for a KB (idempotent); uses `azure-storage-blob` async SDK

- [ ] T006 Create `tests/isolation/Dockerfile`: `FROM python:3.12-slim`; install `uv`; `COPY pyproject.toml uv.lock ./`; `RUN uv sync --extra isolation-tests --no-dev --frozen`; `COPY tests/ ./tests/`; `COPY openkb/ ./openkb/`; `ENV PATH=/app/.venv/bin:$PATH`; default `CMD ["pytest", "tests/isolation/", "-v", "--tb=short"]`

- [ ] T007 Create `tests/isolation/conftest.py`: session-scoped `kb_fixtures` fixture that:
...
- [ ] T009 [US1] Create `tests/isolation/test_scratch_directory_isolation.py`:
...
- [ ] T010 [US2] Create `tests/isolation/test_port_isolation.py`:
...
- [ ] T011 [US3] Create `tests/isolation/test_process_state_isolation.py`:
...
- [ ] T012 [US4] Create `tests/isolation/test_sequential_reuse_safety.py`:
...
- [ ] T013 [US5] Create `tests/isolation/test_concurrent_query_isolation.py`:
